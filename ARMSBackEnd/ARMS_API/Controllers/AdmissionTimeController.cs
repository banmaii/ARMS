...
namespace ARMS_API.Controllers
{
    ...
    public class AdmissionTimeController : ControllerBase
    {
        ...
        [HttpGet("get-admission-time/{id}")]
        public async Task<IActionResult> GetAdmissionTime(int id)
        {
            try
            {
                AdmissionTime response = await _admissionTimeService.GetAdmissionTime(id);
                AdmissionTime responeResult = _mapper.Map<AdmissionTime>(response);
                return Ok(responeResult);
            }
            catch (Exception)
            {
